...
short_seq_prob       = 0.5  # 5% chance to force a short sequence (<= 64 tokens)
...
# ==========================================
# 5. TWO-TIER FUSION MODEL (Fixed & Gated)
# ==========================================
class TwoTierFusionModel(nn.Module):
    def __init__(self, local_layers=3, global_layers=4):
        super().__init__()
        ...
        # NEW: Gating mechanism for competitive routing
        self.gate = nn.Sequential(
            nn.Linear(d_model * 2, d_model),
            nn.Sigmoid()
        )
        
        # Adjusted fusion projection (now taking gated output + CNN features)
    ...
    def forward(self, x, targets=None):
        ...
        if T <= 64:
            ...
            x_local = x[:, -64:]
            # NEW: Token-aligned global context instead of collapsing to one vector
        ...
        # 3. Competitive Gating and Fusion (Local Window Only)
...
@torch.no_grad()
def generate_prediction(model, prompt="Move Up", max_new_tokens=200, temperature=0.4, top_k=50, top_p=0.9, repetition_penalty=1.2):
    model.eval()
