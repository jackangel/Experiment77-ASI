...
bptt_steps     = 64        # Fast unrolling steps for training (3 * 112 = 336 token gradient window)

d_model        = 512
num_layers     = 4         # Increased to 4 for an even Hierarchical split (2 lower, 2 upper)
max_iters      = 3000
eval_interval  = 10
learning_rate  = 3e-4     
warmup_iters   = 400      
...
# ==========================================
# 5. HIERARCHICAL RMT WRAPPER MODEL
# ==========================================
class ImprovedHybridReasoningModel(nn.Module):
    def __init__(self, num_layers=4, mem_size=64):
        super().__init__()
        self.mem_size = mem_size
        self.embedding  = nn.Embedding(vocab_size, d_model, padding_idx=pad_token_id)
        self.embed_drop = nn.Dropout(0.1)

        # Two distinct memory banks for hierarchical reasoning
        self.initial_fast_memory = nn.Parameter(torch.randn(1, mem_size, d_model) * 0.02)
        self.initial_slow_memory = nn.Parameter(torch.randn(1, mem_size, d_model) * 0.02)

        # Split layers into lower (fast/syntax) and upper (slow/semantic)
        self.num_lower = num_layers // 2
        self.lower_layers = nn.ModuleList([
            ImprovedHybridReasoningLayer(d_model) for _ in range(self.num_lower)
        ])
        self.upper_layers = nn.ModuleList([
            ImprovedHybridReasoningLayer(d_model) for _ in range(num_layers - self.num_lower)
        ])
    ...
    def forward(self, x, targets=None, sequential_state=None, memory_state=None):
        ...
        # Unpack hierarchical memory state
        if memory_state is None or memory_state[0] is None:
            fast_mem = self.initial_fast_memory.expand(B, -1, -1)
            slow_mem = self.initial_slow_memory.expand(B, -1, -1)
        else:
            fast_mem, slow_mem = memory_state
        ...
        # --- LOWER LAYERS (Fast Memory + Tokens) ---
        curr_repr = torch.cat([fast_mem, x_embed], dim=1)
        curr_repr = self.embed_drop(curr_repr)
        
        for layer in self.lower_layers:
            curr_repr, sequential_state = layer(curr_repr, sequential_state, padding_mask=full_padding_mask)

        new_fast_mem = curr_repr[:, :self.mem_size, :]
        text_repr = curr_repr[:, self.mem_size:, :]

        # --- UPPER LAYERS (Slow Memory + Processed Tokens) ---
        curr_repr = torch.cat([slow_mem, text_repr], dim=1)
        
        for layer in self.upper_layers:
            curr_repr, sequential_state = layer(curr_repr, sequential_state, padding_mask=full_padding_mask)

        new_slow_mem = curr_repr[:, :self.mem_size, :]
        ...
        return logits, loss, sequential_state, (new_fast_mem, new_slow_mem)
...
def estimate_loss(model):
    ...
    for split in ['train', 'val']:
        ...
        for _ in range(20):
            ...
            if reset: s_s, m_s = None, (None, None)
...
checkpoint_path = 'optimized_hierarchical_rmt_model.pt'
...
print(f"Hierarchical RMT Model initialized. Physical Block: {block_size} (Mem: {mem_size}, Text: {text_size})")
...
seq_state, mem_state = None, (None, None)
...
for iter_num in range(start_iter, max_iters + 1):
    ...
    for step in range(bptt_steps):
        ...
        if seq_state is not None: 
            seq_state = seq_state.detach()
        if mem_state[0] is not None: 
