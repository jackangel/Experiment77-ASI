    ...
    def forward(self, x):
        ...
        for i in range(0, T, self.chunk_size):
            ...
            for layer_idx, layer in enumerate(self.layers):
                ...
                new_mems.append(next_mem[:, -self.mem_len:, :].detach())
                chunk = chunk_out
...
class TwoTierFusionModel(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        # BUG FIX: Added causal mask to prevent lookahead data leaks!
        ...
        # 6. Compute Loss (Decoupled Global Objective)
        loss = None
        if targets is not None:
            if T <= 64:
                target_window = targets
            else:
                target_window = targets[:, -64:]
...
@torch.no_grad()
def generate_prediction(model, prompt="Move Up", max_new_tokens=200, temperature=0.7, top_k=50, top_p=0.9, repetition_penalty=1.2):
    model.eval()
